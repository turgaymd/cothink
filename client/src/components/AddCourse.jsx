...
const AddCourse = ({ setActiveTab }) => {
  ...
  const [course_img, setCourseLink] = useState("");
  const [error, setError] = useState("");

  // const fileInputRef = useRef(null);

  // Fayl seçmə funksiyası
  ...
  const handleCourse = async (e) => {
    ...
    if (!course_title || !course_img) {
      setError("Bütün xanaları doldurun");
      return;
    }

    // LocalStorage-dən mentor_id götür
    // const user = JSON.parse(localStorage.getItem("user"));
    // const mentor_id = user?.mentor_id;

    // if (!mentor_id) {
    //   toast.error("LocalStorage-də mentor tapılmadı!");
    //   return;
    // }
  ...
  return (
    <div className="research-form">
      ...
      <form className="mt-5" onSubmit={handleCourse}>
        ...
        <div>
          <label className="block title font-semibold text-gray-900 pb-2">Dərs Başlığı</label>
          <input
        ...
        <div className="mb-4 mt-4">
          <label className="block title font-semibold text-gray-900 pb-2">Video Linki</label>
          <input
            type="text"
        ...
        {/* {/* <div className="mb-4">
          <label className="block title font-semibold text-gray-900 pb-2">Fayllar</label>
          <div className="flex justify-center items-center bg-gray-100 flex-col gap-3 border border-gray-300 p-5 rounded-2xl">
            <button className="find-btn text-blue-500 px-3 py-2" onClick={handleUpload}>
        ...
        <div className="submit-form mt-5 gap-3 flex flex-col md:flex-row justify-center items-center">
          ...
          <button type="submit" className="w-64 text-white bg-blue-800 px-7 py-4">
            Yadda Saxla
          </button>
        </div>
      </form>

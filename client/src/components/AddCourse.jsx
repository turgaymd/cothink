  ...
  const [courseTitle, setCourseTitle]=useState("")
    ...
    const handleCourse=async(e)=>{
      ...
      return;
        ...
        const newArticle={courseTitle, courseFile, courseCategory}
        try{
      const res= await axios.course("http://localhost/cothinke/server/courses/courseCourse.php", {newArticle},
...
<input type="text" className="w-full form-input border border-gray-300 px-3 py-2 outline-none rounded-lg" placeholder="YouTube, Vimeo və ya digər video linkini daxil edin" onChange={(e)=>setCourseTags(e.target.value)}/>
</div>
...
<input  ref={fileInputRef} type="file"  className="sr-only" onChange={(e)=>setCourseFile(e.target.value)}/>
